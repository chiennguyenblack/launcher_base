...
window.addEventListener('DOMContentLoaded', () => {
    ...
    interval = setInterval(function () {
        ...
        if (timeout == 0) {
            ...
            ipcRenderer.send('close-me', {windowIndex: 'tfaloginvalidation'});
    ...
    submitterBtn.addEventListener('click', function () {
        ...
        showLoading();
        post(config.host + '/api/login/validatetfa', {code: input.value}, validTFACallback);
    });
});
...
var validTFACallback = function (response, request) {
    ...
    if (!empty(response.success) && response.success === true) {
        clearInterval(interval);
        ipcRenderer.send('tfa-login-response', response);
        ipcRenderer.send('close-me', {windowIndex: 'tfaloginvalidation'});
    } else {
