...
window.addEventListener('DOMContentLoaded', () => {
    ...
    //     showLoading();
    //     post(config.host + '/api/deactive2fa', {}, resendTFACallback);
    // });
    ...
    interval = setInterval(function () {
        ...
        if (timeout == 0) {
            ipcRenderer.send('errorbox', [
                'tfadeactivevalidation',
                'Cảnh báo',
                'Quá thời gian xác thực!'
            ]);
            ipcRenderer.send('close-me', {windowIndex: 'tfadeactivevalidation'});
    ...
    submitterBtn.addEventListener('click', function () {
        ...
        showLoading();
        post(config.host + '/api/validatedeactive2fa', {code: input.value}, validTFACallback);
    });
});
