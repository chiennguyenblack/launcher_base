...
window.addEventListener('DOMContentLoaded', () => {
    ...
    interval = setInterval(function () {
        ...
        if (timeout == 0) {
            ipcRenderer.send('errorbox', [
                'tfaexchangevalidation',
                'Cảnh báo',
                'Quá thời gian xác thực!'
            ]);
            ipcRenderer.send('close-me', {windowIndex: 'tfaexchangevalidation'});
    ...
    submitterBtn.addEventListener('click', function () {
        ...
        showLoading();
        post(config.host + '/api/convertCoin/validatetfa', {code: input.value}, validTFACallback);
    });
});
...
var validTFACallback = function (response, request) {
    ...
    if (!empty(response.success) && response.success === true) {
        ...
        ipcRenderer.send('update-coin', [response.data.coin, true]); //coin, reset text = true
        ipcRenderer.send('close-me', {windowIndex: 'tfaexchangevalidation'});
    } else {
