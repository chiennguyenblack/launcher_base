...
{
    public partial class ConvertCoinTwoFactorValidation : Form
    {
        public string username;
        public string token;
        
        int countdown = 120; //seconds
        public ConvertCoinTwoFactorValidation()
        {
            InitializeComponent();
        }
        ...
        {
            timer1.Start();
            Text = this.username + ": Chuyển xu - Xác thực 2 lớp";
        }
        ...
        private string validate(string code)
        {
            ...
            data.code = code;
            Task<string> t = AsyncRequest.createRequestAsync("api/lite/convertCoin/validatetfa", this.token, JsonConvert.SerializeObject(data));
            ...
            {
                Log.AddLog("api/lite/convertCoin/validatetfa: " + responseString);
                MessageBox.Show("Hệ thống gặp lỗi, vui lòng thử lại hoặc liên hệ quản trị viên để được hỗ trợ! (315)", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            Close();
